...
class AbstractApplication {
    ...
    constructor(){
        ...
        // Remove or hide the dat.GUI control box
        if (window.gui.domElement && window.gui.domElement.parentNode) {
            window.gui.domElement.parentNode.removeChild(window.gui.domElement);
        }
        // Or forcibly hide with CSS
        ...
        // Mouse scroll to adjust FOV
        window.addEventListener('wheel', (event) => {
            // Only adjust FOV if zoom is disabled
            if (!this._controls.enableZoom) {
                // event.deltaY > 0 means scroll forward (down) => reduce FOV
                // event.deltaY < 0 means scroll backward (up) => increase FOV
                let delta = event.deltaY > 0 ? 2 : -2;
                this._camera.fov = Math.max(20, Math.min(120, this._camera.fov + delta));
                this._camera.updateProjectionMatrix();
                if (this.fovControl) this.fovControl.setValue(this._camera.fov);
            }
        }, { passive: true });
